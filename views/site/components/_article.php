...
<article class="row mb-5">
    ...
    <div class="col-12 mx-auto">
        <h3><a class="post-title"
               href="<?= Url::to(['article/view', 'id' => $article->id]) ?>"><?= $article->title ?></a></h3>
        <ul class="list-inline post-meta mb-4">
            <li class="list-inline-item"><i class="ti-user mr-2"></i>
                <a href="<?= Url::to(['author/view', 'id' => $article->user->id]) ?>"><?= $article->user->username ?></a>
            </li>
            <li class="list-inline-item">Дата : <?= $formatter->asDate($article->created_at) ?></li>
        ...
        <p><?= $article->description ? Html::encode($article->description) : $article->getShortContent() ?></p> <a
                href="<?= Url::to(['article/view', 'id' => $article->id]) ?>" class="btn btn-outline-primary">Переглянути</a>
    </div>
</article>